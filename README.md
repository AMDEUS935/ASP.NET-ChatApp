...
기존 **PHP + JavaScript(AJAX 폴링)** 기반 1:1 채팅을 **ASP.NET Core + SignalR(WebSocket)** 으로 이식한 프로젝트입니다.  
...
현재 서버는 **실시간 DM/온라인 표시/프로필 이미지**까지는 동작  
(현재는 SignalR Hub에서 메시지를 DB에 저장하지 않고 Group으로 전송하는 구조입니다.)

---

### A. 메시지 영속화 + 히스토리 + 오프라인 전달 

- **Message 테이블 추가(EF Core)**
  
- **채팅 히스토리 API 추가**

---

### B. 서버 방어 추가: Rate limit + 서버 검증
